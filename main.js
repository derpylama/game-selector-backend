...
app.get('/auth/steam/return',
  ...
  (req, res) => {
    // Successful auth
    const token = jwt.sign({ steamid: req.user.id }, process.env.SESSION_SECRET, { expiresIn: '1h' });
    activeTokens.set(token, req.user.id)
    console.log('Authenticated user:', req.user.id);
    res.redirect(`http://localhost:3000/auth/steam/success?token=${token}`);
  }
);
...
app.get('/api/owned-games', authenticateToken, async (req, res) => {
  const steamid = req.user.id; // SteamID64
  ...
  try {
    ...
    if (!r.ok) return res.status(500).json({ error: 'Steam API error' });
